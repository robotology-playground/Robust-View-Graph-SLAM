...
ncams=size(options.cam_left.image,2)+size(options.cam_right.image,2);
kpts=cell(1,ncams); desc=cell(1,ncams); C=[]; k=0;
thickness=options.ncams-1;
ncorners=options.mincorrnr;
mindisp=options.mindisp;
nkeys=0;
for i=1:2:ncams-1	% FIXME: No edge from even to odd
    ...
    for j=i+1:min(i+thickness,ncams)
        if isempty(kpts{j})
