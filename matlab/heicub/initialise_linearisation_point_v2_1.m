...
for j = 2 : ncams
    ...
    if USE_VISION==1 && sum(status)>options.mincorrnr && (STEREO==0||j>2)
        ...
        pwg = test_Emat_v2(pwg, x1, x2, use, options);
...
%     % check if stereo calibration is provided
%     if isfield(options,'R')
%         disp('Using extrinsic stereo calibration parameters');
%         p(1).c
%         p(j).c
%         if mod(p(1).c,2)==1&&p(1).c+1==p(j).c
%             disp('1');
%             xc((j-1)*6+(1:3))= options.T;
%             xc((j-1)*6+(4:6))= R2w(options.R)';
%         elseif mod(p(1).c,2)==0&&p(1).c+1==p(j).c
%             disp('2');
%             xc((j-1)*6+(1:3))= -options.R'*options.T;
%             xc((j-1)*6+(4:6))= R2w(options.R')';
%         end
%     end
