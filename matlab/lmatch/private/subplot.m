...
elseif(isempty(position))
    if (min(thisPlot) < 1)
        ...
        PERC_OFFSET_L = 2*0.09/10;
        PERC_OFFSET_R = 2*0.045/10;
        PERC_OFFSET_B = PERC_OFFSET_L;
        PERC_OFFSET_T = PERC_OFFSET_R;
	SCALE = .9;
        if nrows > 2
            PERC_OFFSET_T = SCALE*PERC_OFFSET_T;
            PERC_OFFSET_B = SCALE*PERC_OFFSET_B;
        end
        if ncols > 2
            PERC_OFFSET_L = SCALE*PERC_OFFSET_L;
            PERC_OFFSET_R = SCALE*PERC_OFFSET_R;
        end
