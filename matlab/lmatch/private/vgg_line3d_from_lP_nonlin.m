...
%   - drmsrel_term ... terminating step of rms of residuals
%   - loglambda_term ... terminating value of lambda (default: 1e10)
%   - loglambda_init ... initial value of lambda
% E.g., vgg_line3d_from_lP_nonlin(...,'loglambda_init',1e-9,'niter_term',5).
%
...
switch size(X,2)
 case 0 
  ...
  % optimization
  p = levmarq({vertcat(P{:}),s,scale,l0,ldelta},...
              [0;0;0;0],...
              varargin{:});
 ...
 case 2
  ...
  % optimization
  p = levmarq(  {vertcat(P{:}),s,scale,l0,ldelta,D,d,c},...
                [0;0;0],...
              varargin{:});
...
%function [a,w] = levmarq(RES,PARAMS,NORMSOLVE,a0,varargin)  % for AZ, function pointers were removed
function [a,w] = levmarq(PARAMS,a0,varargin)
...
while 1
  ...
  while (loglambda < opt.loglambda_term)
    ...
    a = a0 + normsolve(J,e0,w0,10^loglambda);
    [e,w] = F(a,PARAMS{:});
  ...
  %test(2) = ssd0-ssd  >= opt.drmsrel_term^2*ssd0; % bug!
  test(2) = (1-opt.drmsrel_term)^2 >= ssd/ssd0;
