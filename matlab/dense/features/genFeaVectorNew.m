...
% *
% */
function [FeaVector] = genFeaVectorNew(Default, f,fsup,VList,HList,i,near,NeighborFeaList);
...
%PatchSkyBottom = ceil(Default.VertYNuDepth*(1-Horizon));
%if row <= RowskyBottom
...
if size(fsup,2)~=0
   ...
   for l = 1:1
      [Ix Iy] = meshgrid(max(min(HList+shift(l,1),Default.HoriXNuDepth),1),...
                     max(min(VList+shift(l,2),Default.VertYNuDepth),1));
      maskNeibor = sub2ind([Default.VertYNuDepth, Default.HoriXNuDepth], Iy(:), Ix(:));
      if (sum(sum(Iy==1))== size(Iy(:),1) && l ==4) || (sum(sum( Iy == Default.VertYNuDepth ))== size(Iy(:),1) && l ==5)
...
for l = 1:1
    ...
    maskNeibor = sub2ind([Default.VertYNuDepth, Default.HoriXNuDepth], Iy(:), Ix(:));
    %if (sum(sum(Iy==1))== size(Iy(:),1) && l ==4)||(sum(sum( Iy == Default.VertYNuDepth ))== size(Iy(:),1) && l ==5)
    %    maskNeibor2 = sub2ind([Default.VertYNuDepth, Default.HoriXNuDepth], Iy(:)+1, Ix(:));
