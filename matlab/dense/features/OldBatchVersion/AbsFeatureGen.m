...
% *
% */
function [f, fInd] = AbsFeatureGen(Default, H2, SmallSup, HiSupi, SupMaskFlag, FeaMax, fInd)
...
if SupMaskFlag == 1 % Need to use irregular Superpixel Mask
   ...
   for j = 1:NuMask(1)
       for k = 1:NuMask(2);
           ...
           [mask,SupIndex,PixelMask,PatchMask] = makeSubSupMask(gridinfo, HiSupi, SmallSup, [j; k], width, hight);
   ...
   NormalizeValue = conv2(hcol,hrow,ones(ImgResY,ImgResX),'same');
   ...
   for m = 1:17
       ...
       f(:,m) = temp(PixelMask)./NormalizeValue(PixelMask)...
                        ./FeaMax(1,fInd);
       fInd = fInd +1;
   end
