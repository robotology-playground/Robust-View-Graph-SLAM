...
lambda = .000001;  % Damping Gain - Start with 1
kl = 0.1;        % Damping paremeter - has to be less than 1. Changing this will affect rate of convergence ; Recommend to use k1 between 0.01 - 0.05
tol = 1e-9;      % Convergence criterion threshold
Rold = 1000;   % Better to leave this No. big.
...
for n=0:itr % iterate
    ...
    v = v - lambda*(D*H)';
    %fprintf('%d %0.9g %0.9g %0.9g %0.9g %0.9g %0.9g %0.9g %0.9g %0.9g % 0.9g \n',... 
    %    n, v(1), v(2), v(3), v(4), v(5), v(6), v(7), v(8), v(9), norm(R));
    ...
    % satisfied
    if  (n>1)
        %fprintf('%d ', abs(max(2*(v-vold)/(v+vold))));
