function H = test_triangulate_jacobian_inverse_depth(x1, x2, xs, varargin)
%[rho, r1, vis] = test_triangulate_jacobian_inverse_depth(x1, x2, xs)
%
% computes inverse depth jacobian
...
% H = test_triangulate_jacobian_inverse_depth(x1, x2, xs);
...
[RR, dR1, dR2, dR3] = get_rotation_matrix(xs(end-2:end), 'rodrigues');

% Allocate storage for triplet form
N = size(x1, 2);
Hii = zeros(4*N, 1); % 4 diagnonal terms for every inverse depth
Hij = zeros(6*N, 3); % 6 diagnonal terms for every inverse depth
k = 1:6;
for i = 1:N
    % evaluate the jacobian
    hi = compute_inverse_depth_derivatives(x1(:, i), x2(:, i), xs, RR, dR1, dR2, dR3);
    %hi = compute_derivatives(x1, x2, xs, varargin{:});
    % Store block-diagonal terms in triplet form
    ih = getindex_point_triplet(i);
    Hii(ih) = hi(1:4);
    % Store off-diagonal terms in triplet form
    i1 = i; % inverse depth index
    i2 = (4*N + (1:6))'; % pose index
    Hij(k,:) = off_diagonal_triplet_form(hi, i1, i2);
    k = k + 6;
end
[ii, jj] = compute_block_diagonal_triplet_indices(N);
Hii = sparse(ii, jj, Hii, N, 4*N+6);
Hij = sparse(Hij(:,1), Hij(:,2), Hij(:,3), N, 4*N+6);
H = Hii + Hij;

function idx = getindex_point_triplet(i)
idx = (1:4) + (i-1)*4;

function Hij = off_diagonal_triplet_form(H, i1, i2)
% Get off-diagonal block of Y represented in triplet form
rows = ones(1,6); rows = rows(:);
cols = [1,2,3,4,5,6]; cols = cols(:);
Hij = H(5:10);
Hij = [rows + i1(1) - 1, cols + i2(1) - 1, Hij(:)];
...
% function hi = compute_derivatives(x1, x2, xs, varargin)
% if size(x1,1) < 3; x1 = pextend(x1); end
% if size(x2,1) < 3; x2 = pextend(x2); end
% n = size(x1,2);
% % get rotation and its derivatives ('rodrigues', 'exp_skew', 'dcm')
% if isempty(varargin)
%     [R, dR1, dR2, dR3] = get_rotation_matrix(xs(end-2:end), 'rodrigues');
%     R = R';
%     dR1 = dR1';
%     dR2 = dR2';
%     dR3 = dR3';
