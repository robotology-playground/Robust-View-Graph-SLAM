...
function C = test_Emat_v3(C, p1, p2, use, options)
...
if options.bucketsize(1);
    ...
    if 0 % displays image patches
        pts2 = add_lens_distortion(p2(1:2,:), kc2);
        pts2 = pflat(K2*pextend(pts2));
        [im1, im2] = get_image(options, C.edge(1), C.edge(2));
        im{1} = im1;
        im{2} = im2;
        for ii = 1:length(pts2);
            clf;
            view_patch = get_view_patch(im{1}, pts([1 2], ii), 20);
            subplot(1,2,1); imshow(view_patch); title('reference');
            view_patch = get_view_patch(im{2}, pts2([1 2], ii), 20);
            subplot(1,2,2); imshow(view_patch); title('before');
            pause;
        end
    end
