 ...
 ** @param R_pt  3x3 matrix - array of 9 double (out) .
 ** @param dR_pt 9x3 matrix - array of 27 double (out).
 ** @param om_pt 3 vector - array of 3 dobule (in).
 **/

void common::rodrigues(double* R_pt, double* dR_pt, const double* om_pt)
{
    /*
     * Let
     * th = |om|,  r=w/th,
...
void common::irodrigues(double* om_pt, double* dom_pt, const double* R_pt)
{
    /*
     * tr R - 1          1    [ R32 - R23 ]
...
void common::print_matrix(double* A)
{
    //Print the entire (dense) matrix
    std::cout << A << std::endl;
}

void common::print_matrix_sparse(Eigen::SparseMatrix<double> A)
{
    //Print the entire (dense) matrix
    //std::cout << A << std::endl;
...
void common::write_vectorXd_tofile(VecXd v)
{
    size_t n = v.size()/2;
    /* write innovations to file */
    ofstream myfile ("vectorXd.txt");
    if (myfile.is_open())
    {
        for (int i=0; i<n; i++){
            myfile << v.coeffRef(i) <<endl;
        }
        myfile.close();
    }
}
...
//     VecXd temp2(6);
//     for(k=3*n;k<3*n+6;k++){
//         temp2.coeffRef(k-3*n)=xhat.coeffRef(k);
//     }
//     std::cout << temp2 << std::endl;
//     /******************************/

//common obj;
//obj.write_vectorXd_tofile(v);

// std::cout << Y.innerSize() << "  " << Y.outerSize() << std::endl;
// std::cout << H_.innerSize() << "  " << H_.outerSize() << std::endl;
// std::cout << Ht_.innerSize() << "  " << Ht_.outerSize() << std::endl;
// std::cout << R.innerSize() << "  " << R.outerSize() << std::endl;
// std::cout << P.innerSize() << "  " << P.outerSize() << std::endl;


//     /* write matrix B to file */
//     MatrixXd dB= MatrixXd(B);
//     ofstream myfile2 ("B.txt");
//     if (myfile2.is_open()){
//         for (i=0; i<dB.innerSize(); i++){
//             for (j=0; j<dB.outerSize(); j++){
//             //for (SparseMatrix<double,ColMajor>::InnerIterator it(B,i);it;++it){
//                 myfile2 << dB.coeffRef(i,j) << " ";}
//             myfile2 << endl;}
//         myfile2.close();}