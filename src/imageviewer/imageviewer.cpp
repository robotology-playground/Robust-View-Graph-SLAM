...
int main(int argc, char **argv) {
	if (argc != 2) fprintf(stderr, "Usage: imageviewer [filename]\n"), exit(EXIT_SUCCESS);
	uint32_t width = 640, height = 480;

	unsigned char *screenPixels = NULL;
	float *rawPixels = NULL;
	openImageFile(argv[1], width, height, screenPixels, rawPixels);
	...
	glfwSetWindowTitle("Simple Image Viewer");
	...
	initRgbTexture(width, height, rawPixels);
