...
void solve_translations_problem(
    ...
    // Make a random guess solution - x
...
//             exit(1);}}
    ...
    for (int i=0;i<num_edges;++i) {
        ...
        if (loss_width == 0.0) {problem.AddResidualBlock(cost_function,
                NULL, x+3*_edges[2*i+0], x+3*_edges[2*i+1]);}  // No robust loss function
        else {problem.AddResidualBlock(cost_function, new ceres::HuberLoss(loss_width),
                x+3*_edges[2*i+0], x+3*_edges[2*i+1]);}}
